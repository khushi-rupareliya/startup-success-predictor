...
st.caption("AI Startup Intelligence Dashboard")
st.divider()
...
with col1:
    ...
    funding_total_usd = st.number_input("Funding ($)", min_value=0.0)
    age_first_funding_year = st.number_input("Years to First Funding", min_value=0.0)
    age_last_funding_year = st.number_input("Years to Last Funding", min_value=0.0)
    age_last_milestone_year = st.number_input("Years to Last Milestone", min_value=0.0)

with col2:
    age_first_milestone_year = st.number_input("Years to First Milestone", min_value=0.0)
    avg_participants = st.number_input("Avg Investors", min_value=0.0)
...
with tab2:
    ...
    if st.button("Run Simulation"):
        ...
        sim_df["funding_total_usd"] *= (1 + funding_change/100)
        sim_df["milestones"] += milestone_change
...
with tab3:
    ...
    fig.add_trace(go.Bar(
        ...
        y=[
            ...
            failed_df["funding_total_usd"].mean()
        ]
    ))
...
with tab4:
    ...
    st.write("Basic report generated from analysis.")
